...
func (df StagedCompressorDF) Rows() []StageRow {
	if df.rows == nil {
		...
		df.rows = []StageRow{
			...
			getRow("$D_1$", "м", func(df CompressorStageDF) float64 { return df.RotorDF.DOutIn }).FormatString(Round3),
			...
			getRow("$d_3$", "м", func(df CompressorStageDF) float64 { return df.StatorDF.DInOut }).FormatString(Round3),

...
type BuilderTestSuite struct {
	suite.Suite
	pScheme  free3n.ThreeShaftBurnFreeScheme
	pNetwork graph.Network
	scheme   schemes.ThreeShaftsBurnScheme
...
func (s *BuilderTestSuite) TestConsistency() {
	...
	s.InDelta(s.scheme.MidBurner().PStagIn(), s.pScheme.MidBurner().PStagIn(), 1e-6)
	s.InDelta(s.scheme.MidBurner().PStagOut(), s.pScheme.MidBurner().PStagOut(), 1e-6)
	s.InDelta(s.scheme.MidBurner().TStagIn(), s.pScheme.MidBurner().TStagIn(), 1)
	s.InDelta(s.scheme.MidBurner().TStagOut(), s.pScheme.MidBurner().TStagOut(), 5e-1)
	s.approxEqual(initMassRate*bOMR, s.pScheme.MidBurner().MassRateOutput().GetState().Value().(float64), 1e-4)
	s.InDelta(s.scheme.MidBurner().Alpha(), s.pScheme.MidBurner().Alpha(), 1e-3)
